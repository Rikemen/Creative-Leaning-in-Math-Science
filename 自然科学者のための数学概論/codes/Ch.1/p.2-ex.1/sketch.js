...
let scale = 50; //グラフをどれくらいzoom in/zoom outして表示するか？(100の時は100倍原点に近いところだけ見る)
...
function drawGrid(cellSize) {
    ...
    for (let y = - height / 2; y < height / 2; y += cellSize) {
        ...
        if (y != 0) {//原点はメモリ不要なので除外
            ...
            fill(0);
            noStroke();
            text(`${(-1) * y / cellSize * (cellSize / scale)}`, 10, y);
        }
...
function targetFunction(x) {
    ...
    y = 1 / (scaledX - 1);
    let scaledY = (-1) * (y * scale); //(y軸を上向きに取るために-1倍)
    return scaledY;
}
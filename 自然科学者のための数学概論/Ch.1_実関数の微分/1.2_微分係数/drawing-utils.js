...
import { targetMathFunction, calculateDerivative, toScreen, toMathX } from './math-utils.js';
 ...
 */
export function drawMainGraph() {
    stroke(CONFIG.style.colors.graph);
    strokeWeight(2);
    noFill();
 ...
 */
export function drawPointOnGraph(mx) {
    const my = targetMathFunction(mx);
    const pos = toScreen(mx, my);
 ...
 */
export function drawTangentLine(mx) {
    const my = targetMathFunction(mx);
    const slope = calculateDerivative(mx, targetMathFunction);

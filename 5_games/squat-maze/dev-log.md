...
- [x] **Task 6-1: 難易度の段階的上昇（10分）**
  - スコアに応じて `scrollSpeed` を徐々に増加。
  - 通路幅の最低値を徐々に狭める。
  - 障害物の縦の長さをランダムに上げていく（通路の最大80%まで）
  - スコアに応じて障害物の出現頻度を徐々に上げる

- [x] **Task 6-2: 効果音の追加（10分）**
  - 衝突時・ゲームオーバー時・スコア更新時の SE を再生。
  - 音声ファイルは `public/sounds/` に配置。

- [x] **Task 6-3: ゲーム終了画面の修正（10分）**
  - ゲーム終了画面にRetry or Back Homeのボタンを表示する

### フェーズ7: ランキング ニックネーム記録

- [x] ゲーム終了時点で、スコアがランキング10位以内なら、ニックネームを求めるようなUIを生成し、ニックネームとスコアを保存するようにする

### フェーズ8: 改善

- [x] セッティング画面は一旦準備中とする
- [x] ゲームスタートを押した際にml5などのライブラリ読み込み時の画面が真っ白なので、Now loading...の画面を出す

- [x] 肥大化しているファイルがあれば、リファクタリングする
- [x] 今の鼻の座標は画面上の座標とどういう関係にあるのかを確認し、必要なら修正する→鼻のY座標をそのまま画面上のY座標と対応するようになっているのでOK

## フェーズ9: セキュリティ

- [x] fireStoreのルールの確認、修正をする
- [x] 課金リスクの洗い出しと対応（botによる読み書きを防止するため、App Check導入）

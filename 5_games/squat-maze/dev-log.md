...
- [ ] **Task 4-1: スコア（移動距離）の計算と表示（10分）**
  - 毎フレーム `scrollSpeed` を加算してスコア（移動距離）を累計。
  - 画面右下に「LEVEL」 or 「SCORE」として表示。
  - 表示フォーマット: ゼロ埋め 6 桁。
...
- [ ] **Task 5-1: Webカメラ映像の取得（10分）**
  - `p5.createCapture(VIDEO)` でカメラ映像を取得。
  - カメラ映像は非表示（ゲーム画面の邪魔にならないよう）。
  - カメラ許可ダイアログのハンドリング。

- [ ] **Task 5-2: ml5.js MoveNet による姿勢推定（10分）**
  - `ml5.bodyPose('MoveNet')` でモデルをロード。
  - カメラ映像からリアルタイムにキーポイント（nose 等）を取得。
  - コンソールに頭の Y 座標をログ出力して動作確認。

- [ ] **Task 5-3: 姿勢推定とプレイヤー位置の連動（10分）**
  - 頭（nose）の Y 座標をキャンバスの Y 座標にマッピング。
  - マッピング範囲の調整（カメラ解像度 → キャンバス座標系）。
  - マウス操作とのフォールバック切り替え（カメラ未許可時）。

### フェーズ6: 演出と仕上げ

- [ ] **Task 6-1: CRT スキャンライン・ノイズエフェクト（10分）**
  - 半透明の水平線を画面全体にオーバーレイ（モックの `opacity-[0.03]` 準拠）。
  - 微細なカラーフリンジ（RGB ずれ）効果。

- [ ] **Task 6-2: 背景パーティクル・視差効果（10分）**
  - 流れる星（小さい白い丸）を複数レイヤーで描画。
  - レイヤーごとにスクロール速度を変え、奥行き感を演出。

- [ ] **Task 6-3: スタート画面の実装（10分）**
  - `gameState === "ready"` 時に「Navigation Engaged」風のスタートメッセージ表示。
  - 「Abort Mission」ボタンでホーム（`index.html`）へ戻るリンク。
  - スタート待機中のパルスアニメーション。

- [ ] **Task 6-4: 難易度の段階的上昇（10分）**
